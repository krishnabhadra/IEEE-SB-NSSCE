  ...
  return (
    <header
      className={cn(
        ...
        scrolled ? "bg-white py-3 border-b-2 border-black/10" : "bg-transparent py-5"
      )}
    >
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              className="absolute top-0 right-0 w-full sm:w-80 h-screen bg-[#f8f9fa] border-l-4 border-black shadow-[-8px_0_0_0_rgba(0,0,0,0.2)] flex flex-col items-center justify-center gap-6 z-40"
            >
              {navLinks.map((link) => {
                const isActive = pathname === link.href || (link.href !== '/' && pathname?.startsWith(link.href));
                return (
                  <Link
                    ...
                    href={link.href}
                    className={cn(
                      "text-2xl font-black uppercase tracking-widest px-8 py-3 border-2 transition-all w-3/4 text-center",
                      isActive 
              ...
              <div className="w-3/4 h-1 bg-black/10 my-4" />
              
              <Link
                href="https://www.ieee.org/membership/join/index.html"
                target="_blank"
                rel="noopener noreferrer"
                className="px-8 py-4 text-xl font-black uppercase tracking-widest text-white bg-ieee-blue border-4 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all w-3/4 text-center"
                onClick={() => setMobileMenuOpen(false)}
              >
                Join IEEE
              </Link>
